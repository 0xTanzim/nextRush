...
> files, `skills/nextrush/`, `apps/website/content/`, package READMEs, `PUBLISHING.md`, and `wiki/`
...
| Phase 5 | v1 Stable | 13 | 11 | 0 | 2 | 15.4% (T053 verified ☑; T051 ratified ☑ this pass (ESM-only, permanent — see its Verified note); T054 is □ per its own glyph — the earlier "spot-checked plausible ☑" was never reflected in the task entry, corrected here; remainder not re-verified) |
| **Total** | | **64** | **37** | **0** | **27** | **42.2%** |
...
- **Verified (2026-07-17, follow-up — doc-accuracy extended into `@nextrush/types`):** T002's original sweep was scoped to `packages/router/src`; the `@nextrush/types` router header was missed and still claimed "The router uses a radix tree for efficient route matching" — the last remaining radix *drift* in any `.ts` file. Corrected to an accurate segment-trie description ("segment trie keyed by whole path segments, O(k) lookups") by `openspec/changes/fix-router-issues-and-author-radix-rfc` (section 1), commit `00a9546`, alongside a router README "Radix Tree Algorithm" → "Segment Trie Algorithm" fix and the `TrieNode.children` "first character" → "whole path segment" JSDoc correction. Independent re-derivation at that change's HEAD: `grep -rni radix packages/router/src packages/types/src` now returns **2 matches, both accurate references (not drift)** — `router.ts:6` "Segment trie ... not a radix tree" (an explicit negation mirroring the README's allowed contrast) and `match-route.ts:109` "deferred to the radix RFC's benchmark" (a pointer to the new RFC). Neither is a residual claim that the router *is* a radix tree, so T002's requirement holds; but the literal "zero radix token in src" state from the 2026-07-16 rename no longer holds — recorded here for accuracy.
- **Dependencies:** —
...
- **Verified (2026-07-16):** `scripts/check-coverage.ts` enforces 90% lines / 85% branches per package (not repo-wide average). A repo-wide coverage run at wiring time found several packages already below threshold; non-trivial ones (real test-writing gaps, not missing-one-case fixes) were scoped out via an explicit, commented `KNOWN_BELOW_THRESHOLD` exclusion list in `scripts/check-coverage.ts` rather than forced green with speculative tests — **carries forward as open follow-up work, not closed by T006 itself**: `@nextrush/router` (90.02%/78.49%), `@nextrush/runtime` (76.66%/66.86%), and others per the script's inline list. Delivered by `openspec/changes/archive/2026-07-16-fix-dependency-claim-router-naming-coverage-gate` (T006 task group), commit `78e2e06`.
- **Correction (2026-07-17):** The above note's claim that `check:coverage` was "wired... which `verify` now depends on" via `turbo.json`'s `"dependsOn": [..., "validate:bins", "check:coverage"]` was **inaccurate — the wiring never actually executed either task.** Confirmed via `npx turbo run check:coverage --dry-run` and `npx turbo run validate:bins --dry-run`, both returning "No tasks were executed as part of this run" (0/0 tasks) despite exiting 0. Root cause: `check:coverage`, `validate:bins`, and (newly) `validate:esm-only` are root-only `package.json` scripts with no matching script in any workspace member's own `package.json` — Turborepo's per-package task graph has nothing to dispatch them against, so listing them in `verify`'s `dependsOn` silently resolves to zero executions, not a passing check. This means `pnpm verify` in CI (`.github/workflows/ci.yml`) has never actually run the coverage gate or the bin validator, despite this note and the CI config appearing to wire them in. **Fixed in the same pass that added the `module-format-policy` ESM-only check** (see T051): the root `verify` script now explicitly runs `turbo run verify && pnpm validate:bins && pnpm validate:esm-only && pnpm check:coverage` directly, bypassing turbo's task graph for these three root-only scripts; `turbo.json`'s `verify` task's `dependsOn` was corrected to drop the phantom entries. Re-ran directly: `pnpm check:coverage` → real output, 36 packages checked, gate passes with the existing exclusion list intact (confirms the fix didn't change coverage results, only whether the check actually runs).
- **Dependencies:** —
...
- **Domain:** Core / Maintainability · **Packages:** `@nextrush/class`, `@nextrush/router`, `@nextrush/di`, `@nextrush/dev` · **Priority:** P2 · **Effort:** S · **Difficulty:** Easy · **Runtime Impact:** None · **Breaking:** No · **Status:** ☑ Completed
...
- **Verified (2026-07-17):** Delivered by `openspec/changes/improve-router-modularity-and-class-dx-papercuts` (section 1), commit `71c2dc4`. `router.ts` split into 5 new modules along its existing thematic seams (`matching.ts`, `match-route.ts`, `composition.ts`, `middleware-adapter.ts`, `registration.ts`); `redirect.ts`'s multi-method registration was extracted too once `addRoute` itself proved large enough to warrant its own module (design.md D2's explicit escape hatch). Behavior-preserving: the pre-existing public-surface snapshot test passed unmodified before and after (byte-identical exported symbol set — confirmed via `git diff --stat` showing zero changes to that test file across the whole branch), and the full `@nextrush/router` suite stayed green after every individual extraction step, not just once at the end, ending at 212/212. **Partial — smaller, but not yet under the 300-line cap.** `router.ts` measured 473 lines immediately after T014's own commit (48% reduction from 918); it is 525 lines at this change's final HEAD (42.8% reduction from 918) because T016's `.all()`-consolidation logic (a new `_pushAnyMethodRouteDefinition()` helper, JSDoc, and `recordIntrospection` param-threading) was layered directly into the same file afterward — confirmed via `git show 71c2dc4:packages/router/src/router.ts | wc -l` (473) against the final working tree (525). `find packages/router/src -name '*.ts' -not -path '*__tests__*' | xargs wc -l | awk '$1>300'` still returns only `router.ts` at final HEAD — every other file in the package (`registration.ts` 227, `group-router.ts` 185, `segment-trie.ts` 183, `matching.ts` 167, `match-route.ts` 140, `redirect.ts` 97, `route-metadata.ts` 68, `composition.ts` 75, `middleware-adapter.ts` 57, `index.ts` 34) is comfortably under cap. Marked ☑ because the parent proposal's own stated scope ("split `router.ts`... along its existing internal seams... exact split boundaries determined during design") and acceptance criteria were met and independently re-verified; the remaining >300-line state is a disclosed, not silently dropped, open finding (the source change's own task 1.6), consistent with this checklist's Definition of Done requiring acceptance criteria met, not a claim that the global Validation Checklist's repo-wide "no shipping source file > 300 lines" gate is now fully closed. `di`/`dev` were not independently re-checked this pass — unchanged from the 2026-07-15 note; the parent proposal itself found both already under cap before work started, narrower in scope than this task's original framing.
- **Verified (2026-07-17, follow-up — `router.ts` now fully under the 300-line cap):** The partial state disclosed above (525 lines) is resolved. `openspec/changes/fix-router-issues-and-author-radix-rfc` (section 3), commit `e9b0774`, finished the split along the same seams (new internal `dispatch.ts`/`state.ts`/`constants.ts` + further extraction into `registration.ts`/`match-route.ts`/`group-router.ts`; no new structural pattern). Independent re-derivation at this change's HEAD: `find packages/router/src -name '*.ts' -not -path '*__tests__*' | xargs wc -l | awk '$1>300'` reports no individual file over cap (largest is `router.ts` at 298, then `registration.ts` 282) — the earlier `router.ts` over-cap finding is closed. Public surface byte-identical (public-surface.test.ts green), `@nextrush/router` 212/212, and full-repo `turbo run verify` shows no `@nextrush/router` failure. Closes the repo-wide "no shipping source file > 300 lines" gate for `@nextrush/router` specifically (T014's remaining open sub-finding).
- **Dependencies:** —
...
- **Domain:** Decorators / DX · **Packages:** `@nextrush/class` · **Priority:** P2 · **Effort:** XS · **Difficulty:** Easy · **Runtime Impact:** Low · **Breaking:** No · **Status:** ☑ Completed
...
- **Verified (2026-07-17):** Delivered by `openspec/changes/improve-router-modularity-and-class-dx-papercuts` (section 2), commit `38d475e`. `param-resolver.ts` now composes a body-parser remediation hint (mentioning `app.use(json())`) only when the missing parameter's `source === 'body'`, via a new `createMissingParameterError()` helper and an additive, backward-compatible 5th constructor parameter (`messageOverride?: string`) on `MissingParameterError` in `errors.ts`. Every other parameter source (`param`/`query`/`header`/`custom`) keeps the original generic message — confirmed by a dedicated RED-first test guarding against over-broadening the hint. RED confirmed before GREEN (test failed with `AssertionError: expected '...' to match /body-?parser/i` against the pre-fix message, not an unrelated setup error). Test-only file added, no existing test file rewritten: `packages/class/src/__tests__/param-resolver.test.ts` (3 tests — the RED-hint case, an unaffected populated-`ctx.body` happy path, and the non-body-source guard). Full `@nextrush/class` suite re-confirmed independently at this task group's own HEAD: 305/306 passing, `tsc --noEmit` clean; the 1 failure (`registrar.test.ts`'s `surfaces @nextrush/di CircularDependencyError as-is` — a 10s timeout) is confirmed, by directly re-reading this run's own raw output, to be the exact same test name/assertion/timeout as documented pre-existing — see this task group's own 4.1 finding for the full identity check.
- **Dependencies:** —
...
- **Domain:** Router / Decorators · **Packages:** `@nextrush/class`, `@nextrush/router` · **Priority:** P2 · **Effort:** XS · **Difficulty:** Medium · **Runtime Impact:** Low · **Breaking:** No · **Status:** ☑ Completed
...
- **Verified (2026-07-17):** Delivered by `openspec/changes/improve-router-modularity-and-class-dx-papercuts` (section 3), commit `069de37`. `@All()`/`app.all()` — and `router.group(...).all()`, a second independent call site with the identical pattern found only during implementation, not pre-planned — now register exactly one `RouteDefinition` row (`isAnyMethod: true`) instead of one row per enumerated method; all 7 standard verbs still dispatch identically (guarded by the pre-existing "should register all methods with .all()" matching test, re-confirmed green, plus two new single-entry-count tests). **This task's own "Why it matters" risk ("Misleading route tables (OpenAPI/diagnostics)") was a real, confirmed hit, not a hypothetical:** a codebase-wide search for `getRoutes()`/route-table consumers found exactly one production consumer, `@nextrush/openapi`, whose `generate.ts` keyed a generated OpenAPI `PathItem` by each row's `.method` — meaning this change, left unfixed for that consumer, would have silently dropped 6 of 7 methods from any `@All()` route's generated spec. Fixed in the same change (not deferred): `generateDocument()` now checks `route.isAnyMethod` and expands such a row into one operation per standard verb with a distinct `operationId`; a RED-first test proved the pre-fix behavior would have produced exactly that 6-of-7 silent drop before the corresponding GREEN change landed. `@nextrush/class`'s own diagnostics/discovery code was checked too and needed no change (does not branch on row count). New, additive `RouteDefinition.isAnyMethod?: boolean` field in `@nextrush/types`. Full suite re-confirmed independently at this task group's own HEAD: `@nextrush/router` 212/212, `@nextrush/class` 305/306 (1 pre-existing failure, same identity as T015's), `@nextrush/openapi` 22/22 (21 pre-existing + 1 new), all `tsc --noEmit` clean.
- **Dependencies:** —
...
- **Domain:** Build System / TypeScript · **Packages:** core published packages · **Priority:** P3 · **Effort:** M · **Difficulty:** Medium · **Runtime Impact:** Low · **Breaking:** No (additive) · **Status:** ☑ Completed
- **Verified (2026-07-17):** Ratified via `openspec/changes/archive/2026-07-17-module-format-policy` — **RATIFIED: ESM-only, permanent. Dual-publish evaluated and explicitly rejected**, not deferred (see the change's design.md D2/D3 for the full comparison: dual-package hazard on the `reflect-metadata`/`tsyringe` DI path, doubled publishing/test surface across ~35 packages forever, Node ≥22 already covering the strongest CJS-interop case). Pre-flight facts confirmed before ratifying: zero `"require"` conditions in any package's `exports` map (`grep -r '"require"' packages/*/package.json` empty); no top-level `await` in `packages/core/src`, `packages/router/src`, or `packages/nextrush/src` (confirms the Node ≥22.12 native `require(esm)` interop note is accurate for CJS consumers). **Enforced in CI, not just documented**: `scripts/validate-esm-only.ts` (new) fails the build if any package's `exports` map ever gains a `require` condition or `type` drifts from `module`; wired into the root `verify` script. Verified as a real gate, not a tautology: manually injected a `require` condition into `packages/core/package.json`, confirmed the check fails with exit 1 and a specific message, then restored the file (`git diff` clean afterward). Re-ran clean: `pnpm validate:esm-only` → `✅ Module-format policy check passed (36 packages, ESM-only, no CJS)`. **Side finding surfaced while wiring this check**: `check:coverage` and `validate:bins` were listed in `turbo.json`'s `verify` `dependsOn` but never actually executed by `turbo run verify` — Turborepo has no per-package script matching these root-only task names, so they silently resolved to zero tasks run despite exiting 0 (confirmed via `npx turbo run check:coverage --dry-run` → "No tasks were executed"). This means CI's coverage gate and bin validator have not been enforced since they were added, contrary to T006's prior Verified note (corrected there). Fixed alongside this task: the root `verify` script now runs `turbo run verify && pnpm validate:bins && pnpm validate:esm-only && pnpm check:coverage` directly.
- **Dependencies:** T007
...
- **Domain:** Documentation · **Packages:** `apps/website`, all package READMEs · **Priority:** P1 · **Effort:** L · **Difficulty:** Medium · **Runtime Impact:** None · **Breaking:** No · **Status:** ☑ Complete
...
- **Validation Steps:** `grep -ri "hot reload" packages/dev apps/website` → none implying HMR.
...
- **Update (2026-07-29, `audit-unreviewed-security-surface`):**
  ...
  URLs pass through template escaping, an industry-standard limitation, not a code defect) plus
  two narrower ones (F-03: websocket's open-by-default `allowedOrigins`; F-04: unsanitized CR/LF in
  the logger's default message). All four are P2/P3, none P0/P1, and none remediated yet — this
  new report is investigation-only by design (see its own §14). **T064 is still not closed** —
  this pass, like `harden-security-boundaries` before it, resolves a specific named subset (this
