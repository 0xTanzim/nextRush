...
- **Status:** ✅ ALL CONCEPTS DONE (14/14 + routing pilot): `context`·`middleware`·`errors`⊕·`request-lifecycle`·`dependency-injection`·`guards`·`interceptors`·`exception-filters`·`modules`·`application`·`lifecycle`·`runtime-compatibility`·`streaming`⊕·`extensions` (renamed from `plugins`, fixed 54 pre-existing compile errors + updated 10 files' incoming links). `start/quick-start` ✅. **Committed:** `994b2a4`,`cefe578`,`49f438c`,`cd34367`. **Remaining B1 work:** landing page, rest of `start/*` (index/installation/overview/create-nextrush), `start/runtime/*` (5 new onboarding pages), `<RuntimeSupport>` component. **docs:verify total findings: 56→22, all pre-existing/out-of-scope** (community/contributing.mdx, concepts/index.mdx compile errors — flag for a future cleanup node; 2 heading-intent warnings in reference/*). **Tooling signal:** compile-check samples only 15 files by default — raise `sampleSize` at §12. **Lint signal:** forbidden-words is context-blind regex. **Finding for B3:** `reference/class/decorators.mdx` function-style interceptor contradicts source.
