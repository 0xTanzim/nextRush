...
- **Status:** in-progress (concepts: `context`·`middleware`·`errors`⊕·`request-lifecycle`·`dependency-injection`·`guards`·`interceptors`·`exception-filters`·`modules` ✅ (+`routing` pilot) · start: `quick-start` ✅ — validated 2026-07-21). **Committed:** `994b2a4`, `cefe578`. **Tooling signal:** `docs:verify` compile-check samples only 15 files alphabetically by default — raise `sampleSize` at §12 before cutover. **Lint signal:** the forbidden-words CI check is a context-blind `\b(word)\b` regex (e.g. flags "easy" even in honest non-marketing prose like "easy to trip over") — hit twice this batch (exception-filters, modules), both rewritten around it. Consider a smarter check at §12 (phrase-level, not token-level) so future waves don't repeatedly reword around false positives. **Finding for B3:** `reference/class/decorators.mdx` shows a function-style interceptor contradicting source — fix in the Reference wave.
