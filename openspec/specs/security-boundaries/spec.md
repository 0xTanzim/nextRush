...
#### Scenario: An investigation-only change produces a genuine finding in some packages but not others

- **WHEN** this audit's investigation of the named surface concludes with a real P1-P4 finding for
  some packages (e.g. a path-traversal-check gap, a permissive default) while other packages in
  the same review come back clean
- **THEN** each package's conclusion SHALL be reported independently with its own evidence — a
  finding in one package SHALL NOT be read as implying an unstated finding in another, and a clean
  result in one package SHALL NOT be read as implying the whole review found nothing
