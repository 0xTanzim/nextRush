# web-adapters-per-request-work-trim

## Purpose

The Web-platform adapters (`@nextrush/adapter-bun`, `-deno`, `-edge`) avoid the per-request
`ctx.ip` lookup closure when proxies are not trusted, and the redundant `async` frame in
`ctx.next()`, while guaranteeing `ctx.ip` and `ctx.next()` behavior stay byte-for-byte identical
to before and identical across all four adapters (Node + the three) — including the Edge
`cf-connecting-ip` precedence when `trustProxy` is true. These are implementation-level
per-request optimizations mirroring the archived `node-adapter-per-request-work-trim`; the
observable Context behavior they preserve is the cross-adapter regression contract, pinned by
