...
const { NextRushEventSystem } = require('../dist/core/events');
...
async function eventChainExample() {
  ...
  eventSystem.subscribe('user.verification_sent', async event => {
  ...
  eventSystem.subscribe('user.verified', async event => {
    const { email } = event.data;
  ...
  eventSystem.subscribe('user.welcome_sent', async event => {
    const { email } = event.data;
...
async function httpIntegrationExample() {
  ...
  const eventLog = [];
  eventSystem.subscribe('*', async event => {
    eventLog.push(event);
  });
...
async function errorHandlingExample() {
  ...
  eventSystem.subscribe('test.failing', async event => {
  ...
  eventSystem.emit = async event => {
    try {
      await originalEmit(event);
    } catch (error) {
      errors.push(error);
      console.log(`❌ Error processing ${event.type}: ${error.message}`);
    }
  };
