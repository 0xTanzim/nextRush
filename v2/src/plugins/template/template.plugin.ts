...
import type { Application, Middleware } from '@/types/context';
...
export interface TemplatePluginOptions {
  ...
  enableFilePartials?: boolean; // load {{> name}} from viewsDir if not found in partials
  partialExt?: string; // default: .html
}
...
class SimpleTemplateEngine {
  ...
  constructor(options: TemplatePluginOptions = {}) {
    ...
    this.enableFilePartials = options.enableFilePartials !== false; // default true
    ...
    // json helper returns safe (unescaped) string
    this.registerHelper(
      'json',
  ...
  private compile(source: string): CompiledTemplate {
    // Resolve partials: inline map first, then optional file-based
    let tpl = source;
        ...
        case 'each': {
          ...
          if (Array.isArray(iter)) {
            for (let i = 0; i < iter.length; i++) {
              ...
              out.push(...this.renderAST(node.body, child));
            }
  ...
  private evaluateExpression(
    ...
    // Filter style: value | upper | stripHTML
    if (expr.includes('|')) {
  ...
  private tokenize(expr: string): string[] {
    // very small tokenizer: split by spaces, respecting quoted strings
    const tokens: string[] = [];
    let cur = '';
