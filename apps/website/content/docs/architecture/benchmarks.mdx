...
keywords: [architecture, benchmarks, comparison, framework, nextrush]
---
...
<Callout type="info" title="Looking for benchmarks?">
  ...
  [Benchmarks](/docs/getting-started/benchmarking).
</Callout>
...
| Extensibility    | Extension + middleware      | `app.use()`            | Encapsulated plugins | Middleware only       | `app.use()`         |
| Routing          | Segment trie (O(k))         | Linear scan (O(n))     | Radix tree (O(k))    | Radix tree (O(k))     | Via koa-router      |

NextRush's extensibility model is layered: `app.use()` middleware for the vast majority of
cases, plain-function registrars (e.g. `registerControllers`) for wiring subsystems, and the
rare, long-lived `Extension` (via `app.extend()` + `await app.ready()`) for app-scoped services
like an event bus. There is no `Plugin` interface or plugin registration API.
...
<Cards>
  <Card title="From Express" href="/docs/migrate/from-express">
    Replace `req, res, next` with single `ctx` object. Most middleware translates directly.
  </Card>
  <Card title="From Fastify" href="/docs/migrate/from-fastify">
