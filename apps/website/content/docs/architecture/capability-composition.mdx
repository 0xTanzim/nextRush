...
keywords: [architecture, capability, composition, nextrush]
---
...
- **Middleware** answers *"what runs on every request, in what order?"* — a `Middleware` is
...
lifecycle object for something that's actually a plain function, and would force every registrar
...
import { WebSocketServer, type WebSocketOptions } from '@nextrush/websocket';
