...
keywords: [architecture, handbook, nextrush, release]
---
...
_pending changeset_ names. It would never have caught `@nextrush/class` sitting at a hardcoded
...
<Callout type="warn">
  ...
  release. A changeset entry adds nothing for a package with no history to be relative to — it only
  risks producing the exact wrong number this guard exists to catch.
</Callout>
...
shows up as changed at all. If something with zero npm history changed and it's _not_ in
...
| `@nextrush/adapter-bun@1.0.0-beta.0` and `@nextrush/dev@1.0.0-beta.0` both failed with `E409 Conflict` on publish                                                                      | Both package names had been fully unpublished on npm weeks earlier (2026-07-08/09). npm's unpublish policy is permanent for that exact name+version combination — never just a 24/72-hour cooldown once the whole package was unpublished — so republishing the _same_ version string is blocked forever, no matter how much time has passed                                                                                                                 | Bump to a new version string never used before (`1.0.0-beta.1`) — confirm with `npm view <pkg>` first; a `404 ... Unpublished on <date>` response (not a plain 404) is the tell that this is the permanent-reuse block, not a transient registry error  |
| `@nextrush/form-data` and `@nextrush/static` both failed with `E403 Forbidden` on their first-ever publish, while a dozen other brand-new package names in the same run succeeded      | Neither name appeared in `npm access list packages @nextrush` at all beforehand (unlike the unpublish cases above, which return a dated 404) — a genuinely first-time package creation can hit npm's 2FA/granular-access-token publish policy even when the CLI token has write access to every _existing_ package in the scope, because creating a brand-new package name is a stricter security check than publishing a new version of one already claimed | Not automated — needs an interactive `npm publish` (answers the 2FA/OTP prompt directly) or a token with "bypass 2FA" explicitly enabled, done by the account owner before an automated `changeset publish` retry can succeed for that specific package |
...
## GitHub Releases — one release per cycle, not one per package

Changesets publishes packages to npm and pushes a `@<package>@<version>` git tag per package.
That package-level provenance stays. What changed in the v4 era is the **GitHub Releases page**:
the release workflow runs `changesets/action` with `createGithubReleases: false`, then creates
exactly **one** canonical release per cycle — `NextRush vX.Y.Z` — via
`scripts/create-github-release.ts` (run as `pnpm release:github`). npm keeps its 39+ package
versions; GitHub Releases become a product timeline.

Three rules keep that page meaningful:

1. **A canonical release is created only when `nextrush` was published.** GitHub Releases mean
   "a version of the NextRush framework product." A package-only wave (`@nextrush/openapi`,
   `@nextrush/dev`, ...) publishes to npm, writes changelogs, and pushes package git tags —
   but deliberately creates no GitHub Release. The product version is never inferred from the
   highest package version in a wave.
2. **The release body is assembled from the changelogs.** It carries the package matrix, the
   per-package change sections for the released versions (parsed from each package's
   `CHANGELOG.md`), a highlight summary for `nextrush`, and the install line.
3. **CLI / beta / emergency publishes create no GitHub Release.** Running `pnpm changeset
publish` from a laptop is fine — the npm version, the `CHANGELOG.md` entry, and the git tag
   are the source of truth for those. GitHub Releases only reflect what `release.yml` published.

The aggregation logic lives in `scripts/release-notes.ts` — pure, dependency-free functions
(`assembleReleaseBody`, `deriveProductVersion`, `extractChangelogSection`, `groupWaves`,
`buildCanonicalReleasePlans`) with tests in `scripts/__tests__/`. The only `gh` call the forward
path makes is the final `gh release create`; everything else is computed locally.

### Historical releases and the migration

Before v4 each publish produced one GitHub Release per package — 136 releases by August 2026,
which read like an artifact registry rather than a framework history. To fix that, the
`pnpm release:github:cleanup` script (`scripts/migrate-github-releases.ts`) deletes the
package-style releases (git tags preserved) and builds one canonical `NextRush v…` release per
`nextrush`-containing wave, using each wave's underlying git commit as the grouping key. It is
**dry-run by default, mutates only with `--apply`, and is never wired into CI.** Canonical
releases like `NextRush v2.0.0 - Framework` are always preserved untouched.
...
For the package tiers, the CI trigger details, and the GitHub setup this handbook assumes
(secrets, workflow permissions, that kind of thing), see the [Compatibility Matrix](/docs/help/compatibility-matrix),
the package-tier table in the root `packages.json` catalog, and the release workflow
(`.github/workflows/release.yml`) itself. This page is the lifecycle and the reasoning; those are
the reference tables you keep open in a second tab.
