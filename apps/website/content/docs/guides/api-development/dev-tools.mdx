---
title: Dev Tools
description: Set up the NextRush CLI for development with hot reload and production builds with decorator metadata
keywords: [api, dev, development, guides, nextrush, tools]
---

Modern bundlers like esbuild and tsx strip decorator metadata that dependency injection needs. The `@nextrush/dev` CLI provides a hot-reloading dev server and a production build pipeline that preserves this metadata using SWC.

## What You Will Build

A development workflow with a hot-reloading dev server and a production build that compiles TypeScript with SWC, preserving decorator metadata for DI.

## Prerequisites

- Node.js 22+, Bun, or Deno installed
- An existing NextRush project (or a `src/index.ts` entry file)
- `tsconfig.json` — for **class-based** code (`nextrush/class`, DI), enable `experimentalDecorators` and `emitDecoratorMetadata` (the **`class-based`** / **`full`** scaffolds do this automatically). **Functional** scaffolds omit them on purpose because route handlers do not use decorators.

## Installation

<PackageInstall packages={[]} dev={['@nextrush/dev']} />

## Step-by-Step Setup

<Steps>
  <Step>
    ### Add scripts to package.json

    ```json
    {
      "scripts": {
        "dev": "nextrush dev",
        "build": "nextrush build",
        "start": "node dist/index.js"
      }
    }
    ```

  </Step>
  <Step>
    ### Start the dev server

    ```bash
    nextrush dev
    ```

    The CLI auto-detects your entry file, runtime, and TypeScript settings. No configuration required.

  </Step>
  <Step>
    ### Build for production

    ```bash
    nextrush build
    ```

    Compiles TypeScript with SWC, emitting decorator metadata that DI containers require.

  </Step>
</Steps>

## Dev Server Options

```bash
nextrush dev ./src/app.ts --port 4000 --inspect
```

<TypeTable
  title="nextrush dev options"
  types={{
    '--port, -p': { type: 'number', description: 'Server port', default: '8080' },
    '--watch, -w': {
      type: 'string',
      description: 'Additional path to watch (repeatable). With no explicit path, the dev server watches imported files automatically instead.',
      optional: true,
    },
    '--inspect': { type: 'boolean', description: 'Enable Node.js inspector', default: 'false' },
    '--inspect-port': { type: 'number', description: 'Inspector port', default: '9229' },
    '--no-clear': { type: 'boolean', description: "Don't clear screen on start" },
    '--verbose, -v': { type: 'boolean', description: 'Verbose output', default: 'false' },
  }}
/>

The port also respects the `PORT` environment variable when `--port` is not set.

<Callout type="info" title="Watch strategy">
  With no `--watch` flag, the dev server watches imported files automatically — the portable
  default across Node.js, Bun, and Deno. Passing one or more explicit `--watch <path>` flags
  switches Node.js to path-scoped watching (`--watch-path`); if that flag is unsupported on the
  current platform/Node version, the dev server falls back to the automatic mode rather than
  crashing. Bun does not support explicit watch paths and always watches imported files.
</Callout>

## Build Options

```bash
nextrush build --outDir dist --minify --target es2022
```

<TypeTable
  title="nextrush build options"
  types={{
    '--outDir, -o': { type: 'string', description: 'Output directory', default: 'dist' },
    '--target, -t': {
      type: 'string',
      description: 'ES target (es2020, es2021, es2022, esnext)',
      default: 'es2022',
    },
    '--sourcemap': { type: 'boolean', description: 'Generate sourcemaps', default: 'true' },
    '--no-sourcemap': { type: 'boolean', description: 'Disable sourcemaps' },
    '--minify, -m': { type: 'boolean', description: 'Minify output', default: 'false' },
    '--no-decorator-metadata': {
      type: 'boolean',
      description: 'Disable decorator metadata emission',
    },
    '--dts': { type: 'boolean', description: 'Generate .d.ts files', default: 'true' },
    '--no-dts': { type: 'boolean', description: 'Disable .d.ts generation' },
    '--cache': { type: 'boolean', description: 'Use incremental build cache', default: 'true' },
    '--no-cache': { type: 'boolean', description: 'Disable build cache' },
    '--no-clean': { type: 'boolean', description: "Don't clean output directory before build" },
    '--verbose, -v': { type: 'boolean', description: 'Verbose output', default: 'false' },
  }}
/>

## Runtime Behavior

The CLI detects your runtime and adapts automatically:

| Runtime | Dev Server                  | Production Build       | Dev Decorator Metadata |
| ------- | --------------------------- | ---------------------- | ---------------------- |
| Node.js | `@swc-node/register` (SWC)  | `@swc/core` transform  | Emitted                |
| Bun     | `bun --watch` (native)      | `Bun.build()` (native) | Emitted natively       |
| Deno    | `deno run --watch` (native) | `npm:@swc/core`        | Emitted natively       |

<Callout type="info" title="Startup tsconfig validation">
  `nextrush dev` checks `tsconfig.json` when you use TypeScript + SWC. If **only one** of
  `experimentalDecorators` / `emitDecoratorMetadata` is set to `true`, it warns so DI and
  decorators do not half-fail. If **both** are omitted (typical **functional** scaffold), it stays
  quiet — you are not using decorator metadata yet.
</Callout>

## Entry File Detection

The CLI finds your entry file in this order:

1. `package.json` `main` or `module` field (converts `dist/` paths to `src/`)
2. `src/index.ts`, `src/main.ts`, `src/app.ts`, `src/server.ts`
3. `index.ts`, `main.ts`, `app.ts`, `server.ts`

Override by passing the entry directly: `nextrush dev ./src/app.ts`

## Configuration File

The CLI loads settings from `nextrush.config.ts` or a `"nextrush"` field in `package.json`:

```typescript title="nextrush.config.ts"
export default {
  dev: {
    port: 4000,
    watch: ['config'], // additional paths to watch, beyond the automatic imported-files mode
  },
  build: {
    outDir: 'dist',
    target: 'es2022',
    minify: false,
  },
};
```

CLI flags override config file values.

## Verification

Run the dev server and confirm the output:

```bash
nextrush dev
```

You should see:

```
⚡ NextRush Dev Server

  Runtime:  node v22.x.x
  Entry:    src/index.ts
  Port:     8080
  Local:    http://127.0.0.1:8080
  Watching: imported files (auto)
```

<Callout type="warn">
  If you see "TypeInfo not known for X" at runtime while using `nextrush/class`, set both
  `experimentalDecorators` and `emitDecoratorMetadata` to `true` in `tsconfig.json`. Import
  `nextrush/class` (or your entry that loads it) so `reflect-metadata` runs — the `nextrush/class`
  entry applies it for you.
</Callout>

## Programmatic API

For build scripts, monorepo setups, or test harnesses:

```typescript
import { dev, build, detectRuntime } from '@nextrush/dev';

// Dev server
await dev('./src/app.ts', { port: 4000, inspect: true });

// Production build
await build('./src/index.ts', { outDir: 'dist', minify: true });

// Runtime detection
const runtime = detectRuntime(); // 'node' | 'bun' | 'deno'
```

## Next Steps

<Cards>
  <Card
    title="Code Generators"
    description="Scaffold controllers, services, middleware, guards, and routes from the CLI."
    href="/docs/guides/api-development/generators"
  />
  <Card
    title="Class-Based Controllers"
    description="Build APIs with decorators and DI."
    href="/docs/guides/api-development/class-based"
  />
  <Card
    title="Dependency Injection & Scopes"
    description="Understand how DI and scopes work internally."
    href="/docs/concepts/dependency-injection"
  />
  <Card
    title="API Reference — Class Runtime"
    description="Full DI container and decorator API."
    href="/docs/reference/class"
  />
  <Card
    title="Deployment"
    description="Deploy your application to production."
    href="/docs/production/deployment"
  />
</Cards>
