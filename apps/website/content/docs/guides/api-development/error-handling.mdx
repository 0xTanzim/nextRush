...
keywords: [api, development, error, guides, handling, nextrush]
---
...
<Callout type="info" title="Full error surface">
  The classes and middleware used in this guide (`NotFoundError`, `BadRequestError`,
  `ValidationError`, `errorHandler`, `notFoundHandler`, and friends) are re-exported by the
  `nextrush` meta-package — no separate install needed. A few less-common exports
  (`RequiredFieldError`, `TypeMismatchError`, the `badRequest`/`notFound`-style factory functions,
  `getErrorStatus`, `getSafeErrorMessage`) are **not** re-exported by `nextrush` and are imported
  directly from `@nextrush/errors` further down this guide — install it separately
  (`pnpm add @nextrush/errors`) if you need those.
</Callout>
...
<Steps>
  ...
  <Step>
    ...
    **Convenience constructors:**
    ...
    `RequiredFieldError` and `TypeMismatchError` are not re-exported by the `nextrush`
    meta-package — import them directly from `@nextrush/errors`:
...
Create errors without `new`. Only `createError` is re-exported by the `nextrush` meta-package —
the per-status factory functions (`notFound`, `badRequest`, etc.) are imported from
`@nextrush/errors` directly:
...
```typescript
// isHttpError is re-exported by nextrush; getErrorStatus/getSafeErrorMessage are not —
// import those directly from @nextrush/errors.
...
<Callout type="info">
  `catchAsync()` has been removed. It was a no-op wrapper — async errors already propagate to
  `errorHandler()` without it. Remove any remaining calls from existing code.
</Callout>

## Controller-Specific Errors

When using controllers from `nextrush/class`, these additional errors are thrown automatically:
...
const res = await fetch('http://localhost:8080/users/nonexistent');
...
<Cards>
  <Card
    ...
    href="/docs/guides/api-development/validation"
  />
  <Card
