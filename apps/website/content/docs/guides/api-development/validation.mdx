...
keywords: [api, development, guides, nextrush, request, validation]
---
...
<Callout type="info" title="A dedicated validation package exists">
  `@nextrush/validation` provides a `validate()` middleware built specifically for this task — it
  validates the body/query/params against any [Standard Schema](https://standardschema.dev)
...
<PackageInstall packages={['zod', '@nextrush/body-parser', '@nextrush/validation']} />

## Step 0 — Validate with `@nextrush/validation` (recommended)
...
the [`@nextrush/validation` README](https://github.com/0xTanzim/nextRush/tree/main/packages/middleware/validation) for the full options reference.
...
<Steps>
  <Step>
    ...
    ```ts twoslash title="src/schemas/user.ts"
    import { z } from 'zod';
...
type UserInput = z.output<typeof UserInputSchema>;

const data = UserInputSchema.parse({ name: '  Ada  ', email: 'ADA@Example.com', username: 'Ada_Codes' });
//    ^?
```
...
<Cards>
  ...
  <Card
    ...
    href="/docs/reference/class/decorators"
  />
</Cards>
