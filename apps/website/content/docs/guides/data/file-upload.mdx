...
A file upload API using `@nextrush/form-data` — disk storage, size limits, and MIME type validation.
...
<PackageInstall packages={['nextrush', '@nextrush/form-data']} />
...
<Callout type="warn" title="Field name matters">
  `formData()` groups uploaded files by their form field name in `ctx.state.files`. The example
...
<Callout type="info" title="Limits and type checks happen in the middleware">
  `maxFileSize`, `maxFiles`, and `allowedTypes` are enforced by `formData()` itself, before your
