...
keywords: [body, nextrush, parser, reference, request]
---

Parse incoming request bodies before your handlers process them.

<Callout type="info" title="Source & internals">
  [README](https://github.com/0xTanzim/nextRush/blob/main/packages/middleware/body-parser/README.md) ·
  [ARCHITECTURE](https://github.com/0xTanzim/nextRush/blob/main/packages/middleware/body-parser/ARCHITECTURE.md)
</Callout>
...
```ts
function urlencoded(options?: UrlEncodedOptions): Middleware;
```
...
```ts
function text(options?: TextOptions): Middleware;
```
...
```ts
function raw(options?: RawOptions): Middleware;
```
