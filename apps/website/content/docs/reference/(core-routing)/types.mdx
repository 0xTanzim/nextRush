...
keywords: [core, nextrush, reference, routing, types]
---
...
<Callout type="info" title="Source & internals">
  [README](https://github.com/0xTanzim/nextRush/blob/main/packages/types/README.md) ·
  [ARCHITECTURE](https://github.com/0xTanzim/nextRush/blob/main/packages/types/ARCHITECTURE.md)
</Callout>
...
Extensions are the rare (~0.1%) long-lived, app-scoped services that need a boot phase and a
teardown phase — see [Extending NextRush](/docs/concepts/extensions) for when to reach for one
instead of middleware or a registrar.
...
<TypeTable
  ...
  types={{
    app: {
      type: 'ExtensionHost',
      description: 'The subset of Application an extension may use during setup()',
    },
    logger: {
      type: 'Logger',
    ...
    container: {
      type: 'Container',
      description: "The app's per-app DI container, if one was configured",
      optional: true,
    },
    env: {
    ...
    name: {
      type: 'string',
      description: "This extension's own name",
    },
    decorate: {
      type: '<V>(name: string, value: V) => void',
      description: 'Attach a value to the app under `name`. Throws on collision.',
    },
  }}
/>
...
<Cards>
  ...
  <Card title="@nextrush/errors" href="/docs/reference/errors">
    HTTP error classes.
  </Card>
</Cards>
