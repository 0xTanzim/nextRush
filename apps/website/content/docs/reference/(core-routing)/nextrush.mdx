...
<Callout type="info" title="Source & internals">
  [README](https://github.com/0xTanzim/nextRush/blob/main/packages/nextrush/README.md) ·
  [ARCHITECTURE](https://github.com/0xTanzim/nextRush/blob/main/packages/nextrush/ARCHITECTURE.md)
</Callout>
...
<Callout type="info">
  The meta package targets Node.js. For Bun, Deno, or Edge runtimes, install packages individually
...
<Callout type="warn" title="No VERSION export">
  `nextrush` does not export a `VERSION` constant — the meta package
  deliberately omits it to preserve Edge runtime compatibility (no
  `node:fs` access to resolve it at build time). Read the installed
  version from your own `package.json`/lockfile, or see the
  [Compatibility Matrix](/docs/help/compatibility-matrix) for the
  current published version of every package.
</Callout>

## Common Patterns

<details>
<summary>Basic REST API with CRUD operations</summary>
...
| Class runtime (decorators, controllers) | `@nextrush/class`  |
...
- [`@nextrush/core`](/docs/reference) — Application and middleware
- [`@nextrush/router`](/docs/reference/router) — Routing
- [`@nextrush/adapter-node`](/docs/reference/platforms/node) — Node.js adapter
- [`@nextrush/errors`](/docs/reference/errors) — HTTP errors
