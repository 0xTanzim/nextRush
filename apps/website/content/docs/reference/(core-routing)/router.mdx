...
description: Segment trie router with O(k) route matching
keywords: [core, nextrush, reference, router, routing]
---

A segment trie router with O(k) route matching, where k is the number of path segments—not route count.
...
<Callout type="info" title="Source & internals">
  ...
  see also [Router Internals](/docs/architecture/router-internals)
</Callout>
...
NextRush's segment trie router:

- Matches routes in **O(k)** time where k = number of path segments
- Performance is **constant** regardless of route count
- No route ordering bugs—most specific route always wins

<Mermaid chart={`
graph TD
    subgraph trie["Segment Trie"]
        root["/"] --> users[users]
...
// Advanced (segment trie internals)
import { createNode, NodeType, parseSegments } from '@nextrush/router';
import type { HandlerEntry, ParsedSegment, TrieNode } from '@nextrush/router';
...
**O(k) complexity** where k = number of path segments (typically 1–5 segments).
