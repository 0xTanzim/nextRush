...
keywords: [getting, nextrush, one, overview, page, started]
---
...
<div className="not-prose mt-5 flex flex-wrap gap-2">
  ...
  <DocHeroPill>Node · Bun · Deno · Edge · Serverless</DocHeroPill>
...
**Building**, where it ends up **Running**, and **Learning** where to go next. The strip below
stays with you as you scroll; jump to any act:

<ActNav
  acts={[
    { href: 'act-why', n: 1, label: 'Why' },
    { href: 'act-how', n: 2, label: 'How' },
    { href: 'act-building', n: 3, label: 'Building' },
    { href: 'act-running', n: 4, label: 'Running' },
    { href: 'act-learning', n: 5, label: 'Learning' },
  ]}
/>

<DocSectionEyebrow id="act-why">Act 1 · Why</DocSectionEyebrow>
...
same application code running on five different runtimes.
...
[Introduction](/docs/getting-started#where-other-frameworks-trade-off); the rest of this page is about how
...
<RecapCheckpoint
  act="Act 1 · Why"
  items={[
    'NextRush sits in the gap Express (untyped) and NestJS (heavy) leave open — with strict types and optional structure',
    'Four commitments explain every design decision: small core, explicit wiring, capability composition, runtime independence',
    "If you accept any framework's cost (ecosystem, runtime lock-in, learning curve) as the price of entry — you don't need NextRush",
    'All four ideas are made concrete by the four nodes on the request flow, which is what the next act walks through',
  ]}
/>

<DocSectionEyebrow id="act-how">Act 2 · How</DocSectionEyebrow>
...
A request arrives at one place first — the Application owns that place. Every later node on the flow
(Application → Middleware → Router → Context) is something it holds or hands the request down to.

<StageLabel
  active="Application"
  steps={[
    { label: 'Request' },
    { label: 'Application', accent: true },
    { label: 'Middleware', accent: true },
    { label: 'Router', accent: true },
    { label: 'Handler' },
    { label: 'Response' },
  ]}
  note="The first node after a request arrives — the entry point and orchestrator."
/>
...
<GuideAccordion icon="sparkles" iconColor="blue" title="Why is createApp() a function, not a class you extend?" description="A factory returns a closed surface; a class makes you inherit its internals.">
...
The Application owns the pipeline; the pipeline still needs behavior that runs for *every* request
regardless of path — that behavior goes in Middleware, the second node on the flow.
...
<StageLabel
  ...
  note="The pipeline a request flows through before the Router — auth, parsing, logging, all live here."
/>
...
<GuideAccordion icon="refresh-cw" iconColor="green" title="What happens if a middleware never calls ctx.next()?" description="The chain stops — that's how you short-circuit on purpose.">
...
Middleware ran for every request; the next job is to pick the ONE handler responsible for this path —
that's the Router's job, the third node on the flow and where the pipeline hands off to your code.
...
<StageLabel
  ...
  note="Where the pipeline hands off to your code — maps a path to the one handler responsible for it."
/>
...
<GuideAccordion icon="book-open" iconColor="amber" title="Check yourself — which piece picks the handler?" description="Middleware or Router? (open to reveal)">
...
Application, Middleware, and the Router's handler all need to read the same per-request shape and
write back to the same response — that's the fourth and final concept node, Context.
...
<StageLabel
  ...
  steps={[
    ...
    { label: 'Middleware', accent: true },
    { label: 'Router', accent: true },
    { label: 'Context', accent: true },
    { label: 'Handler' },
    { label: 'Response' },
  ]}
  note="What every node above actually reads from and writes to — one fresh object per request."
/>
...
<RecapCheckpoint
  act="Act 2 · How"
  items={[
    'Why the Application exists as a separate piece from the router — it owns the chain, route handlers do not',
    'Why middleware order is execution order, and what skipping `ctx.next()` does (intentional short-circuit)',
    'Why the router is backed by a segment trie instead of a list of `if` statements — lookup cost tracks URL depth',
    'Why `ctx` replaces raw `req`/`res`, and what `ctx.state` is for — a shared namespace every middleware agrees on',
  ]}
/>

<DocSectionEyebrow id="act-building">Act 3 · Building</DocSectionEyebrow>
...
<GuideAccordion icon="settings" iconColor="purple" title="Why is DI the one exception to zero external dependencies?" description="Only the class runtime pulls in reflect-metadata and tsyringe.">
...
You've seen Middleware, Routes, and Context — all per-request. Some capabilities don't run per
request at all (a DB pool, an event bus) and live for the app's whole lifetime instead.
...
<StageLabel
  ...
  steps={[
    ...
    { label: 'Handler' },
    { label: 'Response' },
    { label: 'Lifetime', accent: true },
  ]}
  note="Capability that lives beyond a single request — app-scoped, booted once, torn down on shutdown."
/>
...
Everything so far assumed a handler finished normally. Errors are the escape hatch — what runs when
one cannot.
...
<StageLabel
  ...
  steps={[
    ...
    { label: 'Router', accent: true },
    { label: 'Handler' },
    { label: 'Error', accent: true },
    { label: 'Response' },
  ]}
  note="What happens when a handler can't finish normally — throw, get mapped, surface as clean status + JSON."
/>
...
<RecapCheckpoint
  act="Act 3 · Building"
  items={[
    'Why packages are per-capability, and what installing for "later, maybe" actually costs',
    'Why functional and class are the same engine with different syntax, and when each fits (small vs team-scale)',
    "The decision — middleware, registrar, or extension — and why each exists (per-request, once, app-lifetime)",
    'Why thrown errors beat returned error objects — a forgotten throw is caught for you, a forgotten return is not',
  ]}
/>

<DocSectionEyebrow id="act-running">Act 4 · Running</DocSectionEyebrow>
...
<Cards>
  <Card title="Node.js" href="/docs/getting-started/runtime/node" description="The baseline every other runtime is measured against." />
  <Card title="Bun" href="/docs/getting-started/runtime/bun" description="Runs on Bun.serve()." />
  <Card title="Deno" href="/docs/getting-started/runtime/deno" description="Runs on Deno.serve(), with its permission model." />
  <Card title="Edge" href="/docs/getting-started/runtime/edge" description="Cloudflare Workers and Vercel Edge — no long-running server." />
  <Card title="Serverless" href="/docs/getting-started/runtime/serverless" description="AWS Lambda, Google Cloud Functions, Azure Functions." />
</Cards>
...
<RecapCheckpoint
  act="Act 4 · Running"
  items={[
    'Why the same application code runs on five different runtimes — only the adapter import changes',
    'What the adapter seam is, and why Node-only APIs (`node:fs`, `process`) never belong in application code',
    'The four states a server moves through (Starting → Ready → Handling → Draining), and why draining on SIGTERM matters for zero-drop deploys',
  ]}
/>

<DocSectionEyebrow id="act-learning">Act 5 · Learning</DocSectionEyebrow>
...
Read onward in this order — each stage assumes the one before it:
...
You started with this line:

<MentalModelFlow steps={['Request', 'Application', 'Middleware', 'Router', 'Handler', 'Response']} />

That's the whole framework. Everything in the five acts above is one node on it, one runtime for it,
or one detail about how it's wired. **Read the code and you know what happens on every request** — no
hidden middleware, no auto-wired magic, no runtime you didn't ask for. Start with the
[Task API tutorial](/docs/getting-started/quick-start) to make that line true for your first app, then keep this
diagram in mind as a map the rest of the docs fill in.
