...
keywords: [concepts, core, framework, middleware, nextrush]
---
...
- **Error leakage.** *Threat:* an unhandled error sends a stack trace or file path to the client. *Why:* raw error objects expose internal structure. *Safe default:* wrap the pipeline in an error boundary that maps failures to safe responses — see [error handling](/docs/guides/api-development/error-handling). *Avoid:* returning `err.message` or `err.stack` to clients in production.
