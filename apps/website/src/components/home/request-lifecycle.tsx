    ...
    >
      {stages.map((stage, index) => (
        <div key={stage} className="flex items-center gap-2 sm:gap-3">
          <span className="rounded-lg border border-fd-border bg-fd-card px-4 py-2 font-mono text-sm font-medium text-fd-foreground transition-transform hover:-translate-y-0.5 hover:border-[var(--brand-link)]/40">
            {stage}
          </span>
