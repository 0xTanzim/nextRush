...
/**
 * Allocation micro-benchmark for the HP-5 lazy `ctx.raw` trim
 * (OpenSpec: router-context-final-cleanup, task 4.4 / design D2·D5).
 *
 * Proves the decision gate: on a raw-unread request the shipped `lazy` path
 * allocates measurably less per request than the pre-trim `eager` path, because
 * HP-5 removes the per-request `{ req, res }` wrapper the constructor used to
 * build unconditionally. Runs each variant in its own `--expose-gc` child
 * process for `--runs` runs and reports mean bytes-per-request with a pass/fail
 * verdict (lazy strictly lower).
 *
 * This bench is the gate for whether HP-5 ships or is parked (design D2): it
 * ships only if the wrapper is confirmed no longer allocated on a raw-unread
 * request AND the adapter-node suite stays green.
 *
 * Usage:
 *   node scripts/context-raw-alloc.js                # 5 runs, N=200000
 *   node scripts/context-raw-alloc.js --runs 3 --n 100000
 */
...
const CHILD_SCRIPT = join(import.meta.dirname, 'context-raw-alloc-child.js');
...
function runOnce(variant) {
  ...
  if (result.status !== 0) {
    throw new Error(
      `context-raw-alloc-child.js failed for ${variant}: ${result.stderr || result.stdout}`
    );
  }
...
function main() {
  logHeader('Allocation Micro-Bench — HP-5 lazy ctx.raw (router-context-final-cleanup)');
  logResult('Runs per variant', runs);
  logResult('Requests per run (N)', N.toLocaleString());

  logStep('Measuring lazy (shipped, raw-unread) path...');
  ...
  const verdict =
    lazy.mean < eager.mean ? 'PASS — lazy raw allocates less' : 'FAIL — no allocation reduction';
  ...
  const resultsDir = join(RESULTS_DIR, `context-raw-alloc-${timestamp()}`);
  ensureDir(resultsDir);
  saveResults(resultsDir, 'context-raw-alloc.json', report);
