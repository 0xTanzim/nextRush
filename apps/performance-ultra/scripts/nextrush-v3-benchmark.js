...
/**
 * 🚀 NextRush v3 Dedicated Benchmark Suite
 *
 * A comprehensive benchmarking tool for NextRush v3 performance tracking.
 * Saves results with timestamps for easy comparison over time.
 *
 * Features:
 * - Runs all benchmark tests (hello, params, query, post, mixed)
 * - Saves timestamped results to results/nextrush-v3/YYYY-MM-DD_HH-mm-ss/
 * - Generates detailed markdown reports
 * - Automatically compares with previous run
 * - Tracks performance trends over time
 *
 * Usage:
 *   pnpm bench:v3              # Run full benchmark
 *   pnpm bench:v3 --quick      # Quick 10s tests
 *   pnpm bench:v3 --duration 20 # Custom duration
 *   pnpm bench:v3 --label "v3.0 alpha"  # Add label
 *
...
/**
 * Start the NextRush v3 server
 */
function startServer() {
  return new Promise((resolve, reject) => {
    const serverPath = path.join(PERFORMANCE_DIR, 'servers', 'nextrush-v3.js');
    ...
    server.stderr.on('data', (data) => {
      ...
      // Suppress EPIPE/ECONNRESET errors - normal during high-load benchmarks
...
function generateMarkdownReport(results, config, previous) {
  ...
  lines.push('# 🚀 NextRush v3 Benchmark Report\n');
  ...
  // Footer
  lines.push('---\n');
  lines.push('*Generated by NextRush Benchmark Suite v3.0*\n');
...
function printResults(results, config, previous) {
  ...
  console.log(`${colors.cyan}║${colors.reset}              ${colors.bold}📊 NextRush v3 Benchmark Results${colors.reset}                      ${colors.cyan}║${colors.reset}`);
  console.log(`${colors.cyan}╚══════════════════════════════════════════════════════════════════════╝${colors.reset}`);
  console.log('');
...
async function main() {
  ...
  console.log(`${colors.magenta}║${colors.reset}              ${colors.bold}🚀 NextRush v3 Benchmark Suite${colors.reset}                        ${colors.magenta}║${colors.reset}`);
  console.log(`${colors.magenta}║${colors.reset}                                                                      ${colors.magenta}║${colors.reset}`);
  console.log(`${colors.magenta}╚══════════════════════════════════════════════════════════════════════╝${colors.reset}`);
  console.log('');
  ...
  // Start server
  console.log(`${colors.yellow}🚀 Starting NextRush v3 server...${colors.reset}`);
  let server;
  try {
  ...
  server.kill('SIGTERM');
  await new Promise((resolve) => setTimeout(resolve, 500)); // Allow graceful shutdown
  console.log('');
  ...
  console.log(`   • Compare runs: ${colors.cyan}pnpm bench:v3:compare${colors.reset}`);
  console.log(`   • View history: ${colors.cyan}pnpm bench:v3:history${colors.reset}`);
  console.log('');
}
