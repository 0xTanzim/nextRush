...
See [CSRF API Reference](/docs/api-reference/middleware/csrf) for the full token lifecycle, session binding, and origin-check options.
...
The recommended approach is [`@nextrush/validation`](/docs/api-reference/middleware/validation) — it validates against any Standard Schema library and produces a consistent, secure error shape without hand-written conversion code:
