...
<Callout type="info" title="Upgrading an existing NextRush v3 app?">
  This page covers migrating **from Express**. If you're upgrading a NextRush app that still uses
  `app.plugin()`, `Plugin`, or `controllersPlugin`, see the
  [Extension Model migration guide](https://github.com/0xTanzim/nextrush/blob/main/docs/guides/migration-extension-model.md)
  instead — it covers the breaking change from the plugin system to middleware, registrars, and Extensions.
</Callout>
...
| `fastify-plugin` encapsulation          | Middleware + registrars | No encapsulation scope — see the [Extension Model migration guide](https://github.com/0xTanzim/nextrush/blob/main/docs/guides/migration-extension-model.md) |
| `fast-json-stringify` (AOT)             | Native `JSON.stringify` | Fastify's AOT serialization can outperform on JSON-heavy routes — see [benchmarks](/docs/performance/comparison) for current numbers |
