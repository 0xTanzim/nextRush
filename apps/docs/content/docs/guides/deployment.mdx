...
This compiles TypeScript with SWC, preserves decorator metadata, and outputs JavaScript to `dist/`.
...
COPY tsconfig.json ./
COPY src/ ./src/
RUN pnpm exec nextrush build src/index.ts
...
<Steps>
  ...
  <Step>
    ...
    ```
    Build: pnpm install && pnpm exec nextrush build src/index.ts
...
Deploy NextRush as an Edge Function using `createVercelHandler()` from the Edge adapter.
...
Use `createCloudflareHandler()`, which returns the `{ fetch }` shape Workers expect directly:
...
Bun runs NextRush natively — no build step needed for TypeScript. Use `serve()` from `@nextrush/adapter-bun`, which wraps `Bun.serve()`:
