...
This compiles TypeScript with SWC, preserves decorator metadata, and outputs JavaScript to `dist/`. If your project doesn't use decorators or TypeScript, you can run directly with `node --loader ts-node/esm src/index.ts` or use Bun/Deno natively — no build step required.
