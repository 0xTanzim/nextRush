...
| `--watch`, `-w`   | `src`   | Additional path to watch (repeatable) |
...
| `--target`, `-t`          | `es2022` | Target ES version (`es2020`, `es2021`, `es2022`, `esnext`) |
| `--sourcemap`             | `true`   | Generate source maps                                       |
| `--no-sourcemap`          | —        | Disable source maps                                        |
| `--minify`, `-m`          | `false`  | Minify output                                              |
| `--no-decorator-metadata` | —        | Skip decorator metadata emission                           |
...
await dev('./src/index.ts', options);
```
...
await build('./src/index.ts', options);
```
