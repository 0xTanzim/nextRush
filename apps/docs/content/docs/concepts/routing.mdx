...
You interact with none of this directly. The full internal design — the two-path matcher, executor compilation, the data structures — lives in the [`@nextrush/router` architecture](/docs/internals/router-internals); a concept page teaches the model, not the implementation.
...
The full option reference lives in the [`@nextrush/router` reference](/docs/reference/core/router); this page covers only what each one *means* for matching.
...
- **Wildcard captures.** *Threat:* a `/files/*` route hands you an attacker-controlled remainder. *Why:* `*` captures the whole rest of the URL. *Safe default:* serve files through [`@nextrush/static`](/docs/reference/plugins/static), which guards against traversal. *Avoid:* joining the captured path straight onto a filesystem path.
...
<Cards>
  ...
  <Card title="Mount and group routes" href="/docs/guides/mounting-and-grouping-routes">
  ...
  <Card title="Router architecture" href="/docs/internals/router-internals">
