---
title: Routing
description: Segment trie routing with O(k) lookup, where k is the number of path segments
---

NextRush uses a segment trie router for O(k) lookup performance — where k is the number of path segments, not the number of registered routes.
...
<Tabs items={['app.route() (Recommended)', 'router.mount()', 'Classic Pattern']}>
  <Tab value="app.route() (Recommended)">
...
Alongside middleware and the handler, a route's argument list can carry a metadata marker — an object with no runtime behavior that never enters the executed chain. `endpoint()` creates one directly; `validate()` (from [`@nextrush/validation`](/docs/api-reference/middleware/validation)) attaches one to itself as a non-enumerable property, so it works as both a functioning middleware and a metadata source at once.

```typescript
import { createRouter, endpoint } from '@nextrush/router';
...
router.post(
  ...
  endpoint({ summary: 'Create a user', responses: { 201: User } }), // contributes docs
  (ctx) => {
    ctx.status = 201;
    ctx.json(ctx.body);
  }
...
The router reads every contribution at registration time, merges them in argument order into one `RouteMetadata` object, and stores the result alongside the route's method and resolved path. Call `router.getRoutes()` to get the full list as `RouteDefinition[]` — a read projection meant for tooling, never touched on the request hot path.

<Callout type="info">
  The router stores raw Standard Schema objects and generic facts (`summary`, `tags`,
  `deprecated`, `visibility`) — never renderer-specific shapes. `@nextrush/openapi` is the first
  consumer that turns this into an OpenAPI document, but the metadata itself has no OpenAPI
  concepts baked in.
</Callout>

`RouteMetadata` fields:

| Field | Contributed by | Meaning |
| --- | --- | --- |
| `request.body` / `.query` / `.params` | `validate()` | Standard Schema for that request part |
| `responses` | `endpoint({ responses })` | Standard Schema per HTTP status code |
| `summary` / `description` / `tags` / `deprecated` | `endpoint()` | Documentation facts |
| `visibility` | `endpoint({ visibility: 'internal' })` | `'internal'` routes are excluded from public specs and SDKs |

A route with no `validate()` or `endpoint()` call has no metadata at all — `getRoutes()` still lists it, with `metadata` left `undefined`. Nothing about route registration or dispatch requires metadata to be present.
...
**Wildcard not at end** — `router.get('/*/end', handler)` does not throw. The parser stops at the wildcard and silently drops everything after it, registering the route as `/*`. Always put wildcards as the final segment.

**Double-prefixing** — using `createRouter({ prefix: '/api' })` and then `app.route('/api', router)` causes paths like `/api/api/users`.

## Performance Notes

Lookup time is O(k) where k is the number of path segments (typically 3–5 for REST APIs). Route count does not affect lookup time. Static routes use an additional O(1) hash map fast path, bypassing tree traversal entirely.
...
<Cards>
  ...
  <Card title="@nextrush/validation" href="/docs/api-reference/middleware/validation">
    The middleware that contributes request schemas to route metadata.
  </Card>
  <Card title="@nextrush/openapi" href="/docs/api-reference/plugins/openapi">
    Generates an OpenAPI 3.1 document from route metadata.
  </Card>
  <Card title="@nextrush/router API" href="/docs/api-reference/core/router">
