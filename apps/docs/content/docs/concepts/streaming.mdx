...
- **Why NextRush chose this** — response streaming here is strictly server-to-client; a use case needing the client to send messages back over the same connection belongs on [`@nextrush/websocket`](/docs/reference/plugins/websocket) instead of stretching one of these three writers to do a job they weren't built for.
...
<Cards>
  ...
  <Card title="@nextrush/stream reference" href="/docs/reference/plugins/stream">
