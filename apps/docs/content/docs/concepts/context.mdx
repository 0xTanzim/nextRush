...
<Callout type="tip" title="Two Syntaxes">
...
| `ctx.url` | `string` | Full URL with query |
...
| `ctx.state` | `Record<string, unknown>` | Shared state |
| `ctx.get(name)` | `string \| undefined` | Get request header |
| `ctx.set(name, value)` | `void` | Set response header |
| `ctx.json(data)` | `void` | Send JSON response |
| `ctx.send(data)` | `void` | Send text/buffer/stream |
| `ctx.html(content)` | `void` | Send HTML response |
| `ctx.redirect(url, status?)` | `void` | Redirect |
| `ctx.throw(status, message?)` | `never` | Throw HTTP error |
| `ctx.assert(condition, status, message?)` | `void` | Assert or throw |
| `ctx.next()` | `Promise<void>` | Call next middleware |
