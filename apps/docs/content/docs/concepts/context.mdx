...
Request headers as a read-only object. In Node.js, header keys are lowercased.
...
- Uses current `ctx.status` (defaults to 200)

### `ctx.send(data)`

Send a response body. Handles strings, `Buffer`, `Uint8Array`, `ArrayBuffer`, Node.js streams, Web `ReadableStream`, objects (delegates to `json()`), and `null`.
...
Set a response header. Accepts `string`, `number`, or `string[]` for multi-value headers.
...
Get or set the response status code. Defaults to `200`.

```typescript
ctx.status = 201; // Set status
console.log(ctx.status); // Get status (200 if not set)
```
