...
Request properties are read-only. See [Quick Reference](#quick-reference) for the full property table.
...
Response methods are documented in full in [Quick Reference](#quick-reference).
...
Context itself has no configuration surface — its behavior is fixed by the adapter that creates it. The one adapter-level setting that changes Context behavior is `proxy`:
...
<TypeTable
  title="Application Options Affecting Context"
  types={{
    proxy: {
      type: 'boolean',
      description: 'When true, ctx.ip trusts X-Forwarded-For / X-Real-IP headers from the incoming request',
      default: 'false',
    },
  }}
/>

With `proxy: false` (the default), `ctx.ip` is read directly from the underlying socket/connection and forwarding headers are ignored. There is no per-request or per-route override — `proxy` is set once when the app is created.

## Performance Notes

- **Query parsing runs once per request.** `ctx.query` is parsed from the URL when the context is created, not lazily on first access.
- **`ctx.get()` is case-insensitive by design** — it does a lookup independent of header casing, which costs slightly more than direct `ctx.headers[key]` access. Prefer `ctx.headers` in hot paths where casing is already known.
- **`ctx.send()` performs type detection** on every call (string vs. Buffer vs. stream vs. object) to choose the right `Content-Type` and serialization path. Use `ctx.json()` or `ctx.html()` directly when the response type is known ahead of time to skip that dispatch.
- **No deep cloning.** Context properties (`params`, `query`, `state`) are plain objects passed by reference — mutating them mutates the live request state for the remainder of the pipeline.

## Security Considerations

- **Never trust `ctx.headers` or `ctx.body` without validation.** Both come directly from the client. Validate shape and type before use — a missing body-parser or malformed input surfaces as `undefined` or an unexpected shape, not a validation error.
- **`ctx.raw` bypasses all cross-runtime guarantees.** It exposes the platform-specific request/response object (`IncomingMessage`/`ServerResponse` on Node.js, `Request`/`Response` on Bun/Deno/Edge). Code that reads `ctx.raw` only works on the runtime it was written for.
- **`ctx.ip` requires explicit proxy configuration.** With `proxy: false` (default), forwarding headers are ignored and `ctx.ip` reflects the real socket peer. Setting `proxy: true` trusts `X-Forwarded-For`/`X-Real-IP` — only enable this behind a proxy you control, since these headers are otherwise attacker-controlled and trivially spoofed.
...
<Cards>
  ...
  <Card title="@nextrush/core" href="/docs/api-reference/core">
