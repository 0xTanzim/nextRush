...
- **`ctx.ip` requires explicit proxy configuration.** With `proxy: false` (default), forwarding headers are ignored and `ctx.ip` reflects the real socket peer. Setting `proxy: true` trusts `X-Forwarded-For`/`X-Real-IP` — only enable this behind a proxy you control, since these headers are otherwise attacker-controlled and easy to spoof.
