...
1. **The adapter builds the context.** The platform hands the adapter a native request; the adapter constructs one `ctx`, parsing method, path, query, headers, and client IP up front. How that construction differs per runtime is the [adapter's job](/docs/internals/adapters) — your code sees the same `ctx` everywhere.
