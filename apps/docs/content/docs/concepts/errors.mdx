...
The hierarchy is why throwing is enough: any node in this tree is an `HttpError` the pipeline knows how to serialize. `BadRequestError`, `NotFoundError`, and the rest are named presets over the same base — the full set lives in the [`@nextrush/errors` reference](/docs/reference/core/errors). How the pipeline installs its `catch` is the [middleware pipeline's job](/docs/internals/middleware-internals), not the handler's.
...
<Cards>
  ...
  <Card title="@nextrush/errors reference" href="/docs/reference/core/errors">
    Every error class, factory, and handler option, with signatures.
  </Card>
  <Card title="Middleware pipeline internals" href="/docs/internals/middleware-internals">
