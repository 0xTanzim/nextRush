...
flowchart TB
    ...
    subgraph "Layer 3: Controllers Registrar"
        F[registerControllers] --> G[Read metadata via getControllerDefinition]
...
The `@nextrush/decorators` package exports reader functions. The controllers registrar uses `getControllerDefinition`:

```typescript
import { getControllerDefinition } from 'nextrush/class';
...
The `Container` interface exposes these methods:
...
## How the Controllers Registrar Works

`registerControllers()` connects decorators, DI, and the router. It reads `app.router` (required) and `app.container`:

### Class Guards Resolved from DI

Guards implementing `CanActivate` are classes, so they can declare their own dependencies through
the constructor — useful when a guard needs a service like token verification:

```typescript
import { type CanActivate, type GuardContext } from 'nextrush/class';
...
@UseGuard(RoleGuard) // Class reference, resolved from DI
@Controller('/admin')
class AdminController {}
```

At request time, the controllers registrar resolves `RoleGuard` from the same DI container as
controllers and services, so `authService` is injected automatically — no manual instantiation.

<Mermaid chart={`
sequenceDiagram
    participant App as Application
    participant Reg as registerControllers
    participant Dec as Decorators
    participant DI as Container
    participant Router as Router

    App->>Reg: await registerControllers(app, { root: './src' })
    Reg->>Reg: Discover or receive controller classes

    loop For each controller
        Reg->>Dec: getControllerDefinition(controller)
        Dec-->>Reg: { target, controller, routes, params }
        Reg->>Dec: getAllGuards(target, methodName)
        Dec-->>Reg: Guard[]

        loop For each route
            Reg->>Reg: buildHandler(guards, paramMetadata)
            Reg->>DI: container.resolve(controller) [deferred to request time]
            Reg->>Router: router.get/post/...(fullPath, handler)
        end
    end
...
- **Readers**: `getControllerDefinition`, `getControllerMetadata`, `getRouteMetadata`, `getParamMetadata`, `isController`, `getResponseHeaders`, `getRedirectMetadata`
...
- **Decorators**: `Service`, `Repository`, `Optional`
...
- **Errors**: `CircularDependencyError`, `DependencyResolutionError`, `InvalidProviderError`
...
- **Registrar**: `registerControllers` async function
...
You can use decorators without DI, or DI without controllers. The controllers registrar is the only package that depends on both.
...
<Cards>
  ...
  <Card title="Controllers Package" href="/docs/api-reference/plugins/controllers">
    Full API reference for the controllers registrar.
  </Card>
  <Card title="DI Package" href="/docs/api-reference/di/di">
