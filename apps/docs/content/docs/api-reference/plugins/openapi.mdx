...
- [`@nextrush/validation`](/docs/api-reference/middleware/validation) — the middleware that contributes request schemas to route metadata
- [Routing](/docs/concepts/routing) — how the router matches and registers routes
- [`@nextrush/stream`](/docs/api-reference/plugins/stream) — streaming routes are not represented in OpenAPI; there is no schema for a token stream
