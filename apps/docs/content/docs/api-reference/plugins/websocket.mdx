...
Three steps: create the WebSocket server, register route handlers, then attach to the HTTP server after it starts listening. `listen()` returns a `ServerInstance` — pass its `.server` property (the underlying `node:http` `Server`) to `wss.attach()`, not the `ServerInstance` wrapper itself.
...
<Callout type="warn" title="Requires an app.listen() method">
  `withWebSocket()` calls `app.listen(port, callback)` internally and expects that call to return a
  raw `node:http` `Server`. NextRush's `Application` (from `@nextrush/core`) has no `listen()`
  method — only the adapter package does, and it returns a `ServerInstance` wrapper, not a raw
  `Server`. Use the manual `listen()` + `wss.attach(server.server)` pattern shown above with
  NextRush; `withWebSocket()` is for host objects that expose `listen()` directly, matching Express's
  shape.
</Callout>
...
- [@nextrush/events](/docs/api-reference/plugins/events) — Event emitter plugin
