...
- [`@nextrush/openapi`](/docs/api-reference/plugins/openapi) — documents regular JSON routes; streaming routes are not represented in OpenAPI (no schema for a token stream)
