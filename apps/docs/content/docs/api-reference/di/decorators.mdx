...
<PackageInstall packages={['@nextrush/decorators']} />
...
// Get design-time parameter types for a method (requires emitDecoratorMetadata)
const paramTypes = getMethodParameterTypes(UserController.prototype, 'findOne');
// [String, Context]

// Get design-time return type for a method (requires emitDecoratorMetadata)
const returnType = getMethodReturnType(UserController.prototype, 'findOne');
// Promise
```

`getMethodParameterTypes()` and `getMethodReturnType()` read the `design:paramtypes` and `design:returntype` reflect-metadata keys emitted by TypeScript when `emitDecoratorMetadata: true`. The controllers plugin uses these to resolve parameter types for constructor and method injection.
