...
Fast bundlers like esbuild and tsup strip TypeScript decorator metadata, which breaks constructor-based dependency injection (`@Service`, `@Controller`). `nextrush build` uses SWC specifically to preserve it. See [`@nextrush/di`](/docs/api-reference/di/di) for why DI needs this metadata.
...
<TypeTable
  ...
  types={{
    ...
    '--watch, -w': { type: 'string', description: 'Additional path to watch (repeatable)' },
