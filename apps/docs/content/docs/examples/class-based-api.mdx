...
<PackageInstall
  packages={['nextrush', '@nextrush/controllers', '@nextrush/cors', '@nextrush/body-parser']}
...
import { Controller, Get, Post, Put, Patch, Delete, Body, Param, Ctx } from 'nextrush/class';
...
async function main() {
  ...
  // Auto-discover all controllers in ./src — registrar, must be awaited
...
When the app starts with `debug: true` added to the registrar options, you'll see:
...
No manual imports or controller arrays needed. Add a new controller file anywhere in `./src` with the
...
- [DI & Decorators Architecture](/docs/concepts/di-decorators) — How it works internally
- [API Reference — Controllers](/docs/api-reference/plugins/controllers) — Full controllers plugin API
- [Security Best Practices](/docs/guides/security) — Helmet, CORS, and rate limiting
