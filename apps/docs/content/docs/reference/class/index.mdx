...
used to be three separate packages — `@nextrush/decorators`, `@nextrush/controllers`, and a
re-export of `@nextrush/di` — into one import surface: `nextrush/class`.

<Callout type="warn" title="@nextrush/decorators and @nextrush/controllers are deprecated">
  Both packages are compatibility shims that re-export from `@nextrush/class`. New code should
  import from `nextrush/class` directly. See [Deprecated packages](#deprecated-packages) below.
</Callout>
...
`@nextrush/decorators` and `@nextrush/controllers` still exist as npm packages, but each is a
thin compatibility shim that re-exports the corresponding symbols from `@nextrush/class`. They
receive no independent development — fix an issue in `@nextrush/class` and both shims pick it up
automatically.
