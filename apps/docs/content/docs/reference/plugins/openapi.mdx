...
- [`@nextrush/validation`](/docs/reference/middleware/validation) — the middleware that contributes request schemas to route metadata
- [Routing](/docs/concepts/routing) — how the router matches and registers routes
- [`@nextrush/stream`](/docs/reference/plugins/stream) — streaming routes are not represented in OpenAPI; there is no schema for a token stream
