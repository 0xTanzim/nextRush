...
<Callout type="warn">
  ...
  targets Node.js 22+ only; for Bun, Deno, or edge runtimes, use their native WebSocket APIs.
</Callout>
...
<Callout type="info" title="No withWebSocket() helper">
  There is no `withWebSocket()` integration helper — it required an
  `app.listen(port, callback)` method that `Application` never had, and had
  no real callers. Use the `listen()` + `wss.attach(server.server)` pattern
  from Minimal Usage above; it is the only supported integration path.
</Callout>
...
- [@nextrush/events](/docs/reference/plugins/events) — Event emitter plugin
