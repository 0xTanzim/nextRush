...
description: Production-grade request rate limiting with multiple algorithms, tiered limits, and enterprise security features.
---
...
```mermaid
flowchart TD
    A["📥 Incoming Request"] --> B{"Identify Client<br/>(IP, API key, user)"}
    B --> C{"In Whitelist?"}
    C -->|Yes| G["✅ Skip to Handler"]
    C -->|No| D{"In Blacklist?"}
    D -->|Yes| E["Apply Stricter Limits"]
    D -->|No| F["Normal Limits"]
    E --> H{"Check Algorithm<br/>(tokens/count)"}
    F --> H
    H -->|Has Quota| I["✅ Decrement & Continue"]
    H -->|No Quota| J["❌ 429 Too Many Requests"]
    I --> G
    J --> K["📤 Return with Headers<br/>RateLimit-Remaining: 0<br/>Retry-After: 30"]
```
...
stateDiagram-v2
    direction LR
    [*] --> Full: Initialize
    Full --> Available: Request (consume token)
    Available --> Available: Request (has tokens)
    Available --> Empty: Request (last token)
    Empty --> Empty: Request (REJECT 429)
    Empty --> Available: Time passes (refill)
    Available --> Full: Time passes (refill to max)

    note right of Full: burstLimit tokens
    note right of Empty: 0 tokens - blocked
```
...
```mermaid
timeline
    title Rate Limit Window Comparison
    section Fixed Window
        Window 1 (0:00-1:00) : 100 requests allowed
        Window 2 (1:00-2:00) : 100 requests allowed
        ⚠️ Boundary issue : 200 requests in 2 seconds at boundary
    section Sliding Window
        Any 60-second period : Always ≤100 requests
        No boundary exploit : Weighted average prevents spikes
```
