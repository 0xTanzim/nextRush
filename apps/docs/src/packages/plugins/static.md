...
description: Serve files from disk with production-grade security, caching, and performance.
---
...
flowchart TB
    subgraph Request["📥 Incoming Request"]
        REQ["GET /images/logo.png"]
    end

    subgraph Gateway["🔒 serveStatic({ root: './public' })"]
        V1["1. Validate path<br/>(no traversal)"]
        V2["2. Check symlinks<br/>(blocked by default)"]
        V3["3. Apply dotfile policy"]
        V4["4. Stat file<br/>(or try extensions)"]
        V5["5. Check conditional headers<br/>(304?)"]
        V6["6. Check range headers<br/>(206?)"]
        V7["7. Stream file<br/>with proper headers"]
    end

    subgraph Response["📤 Response"]
        RES["200 OK<br/>Content-Type: image/png<br/>ETag: W/'1a2b3c...'<br/>Cache-Control: public, max-age=3600<br/>X-Content-Type-Options: nosniff<br/>[file bytes]"]
    end
...
sequenceDiagram
    ...
    Note over M: Step 1: Path Validation
    M->>M: Decode URL
    M->>M: Check for traversal (..)

    Note over M: Step 2: Symlink Check
    M->>FS: lstat('./public/style.css')
    FS-->>M: { isSymbolicLink: false }

    Note over M: Step 3: File Stat
    M->>FS: stat('./public/style.css')
    FS-->>M: { size: 4096, mtime: Date }

    Note over M: Step 4: Conditional Check
    M->>M: Compare ETag/If-None-Match
    alt File unchanged
        M-->>C: 304 Not Modified
    else File changed
        Note over M: Step 5: Range Check
        M->>M: Parse Range header
        alt Has Range
            M->>FS: createReadStream(start, end)
            FS-->>M: Partial content
            M-->>C: 206 Partial Content
        else No Range
            M->>FS: Read/Stream file
            FS-->>M: File bytes
            M-->>C: 200 OK + Headers + Body
        end
    end
```
...
flowchart LR
    ...
    subgraph Production["🚀 Production"]
        subgraph Assets["Fingerprinted Assets"]
            A1["maxAge: 31536000"]
            A2["immutable: true"]
            A3["1 year cache"]
        end
...
flowchart TB
    ...
    subgraph Policy["Dotfiles Policy"]
        direction TB
...
flowchart LR
    ...
    subgraph Fallback["Extension Fallback Search"]
        direction TB
        F1["Try /about"]
        F2["Try /about.html"]
        F3["Try /about.htm"]
...
```mermaid
flowchart TB
    subgraph Request["Incoming Requests"]
        R1["GET /app.js"]
        R2["GET /dashboard"]
    end

    subgraph Static["serveStatic (fallthrough: true)"]
        S1["File exists?"]
    end

    subgraph SPA["SPA Fallback Handler"]
        F1["Serve index.html"]
    end

    R1 --> S1
    R2 --> S1
    S1 -->|Yes| RES1["200 OK (app.js)"]
    S1 -->|No| F1
    F1 --> RES2["200 OK (index.html)"]
```
...
```mermaid
flowchart TB
    REQ["GET /assets/style.css"]

    subgraph Pipeline["Middleware Pipeline (First Match Wins)"]
        direction TB
        M1["serveStatic(/assets)<br/>maxAge: 1 year, immutable"]
        M2["serveStatic(/uploads)<br/>maxAge: 0"]
        M3["serveStatic(/public)<br/>maxAge: 1 hour"]
    end

    RES["Response with<br/>appropriate caching"]
...
```mermaid
flowchart LR
    subgraph Attacks["🚫 Blocked Attack Vectors"]
        A1["/../../../etc/passwd"]
        A2["/%2e%2e/%2e%2e/etc"]
        A3["/..%5c..%5cetc"]
        A4["/file.txt%00.html"]
    ...
    subgraph Defense["🛡️ Security Layers"]
        ...
        D3["Check for '..'"]
        D4["Verify in Root"]
        D5["Block Null/Double"]
    end

    subgraph Result["✅ Safe Result"]
        R["403 Forbidden<br/>or<br/>Valid File Path"]
    end
...
```mermaid
flowchart TB
    subgraph Request["File Request"]
        REQ["GET /file"]
    end

    subgraph Check["Size Check"]
        SIZE{"File size<br/>< 1MB?"}
    end

    subgraph Small["Small Files"]
        SM1["Read into memory"]
        SM2["Single response"]
        SM3["OS file cache"]
    end

    subgraph Large["Large Files"]
        LG1["Stream to response"]
        LG2["Memory efficient"]
        LG3["Range request support"]
    end

    subgraph Conditional["Conditional Request?"]
        COND{"ETag/If-Modified<br/>match?"}
    end

    R304["304 Not Modified<br/>(no body)"]
    R200["200 OK<br/>(with body)"]
