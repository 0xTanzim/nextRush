---
title: Event Emitter
description: Type-safe, async-ready event system for decoupled application architecture.
---
...
flowchart LR
    ...
    subgraph EventEmitter["📋 EventEmitter"]
        E["user:created"]
    end
...
sequenceDiagram
    ...
    App->>E: emit('user:created', data)

    rect rgb(255, 243, 224)
        Note over E: 1. Validation
        E->>E: Validate event name (string, ≤256 chars)
    end

    rect rgb(224, 247, 250)
        Note over E: 2. Collect Handlers
        E->>E: Get exact match handlers
        E->>E: Get pattern match (user:*)
        E->>E: Get wildcard handlers (*)
    end

    rect rgb(232, 245, 233)
        Note over E,H: 3. Execute (Promise.allSettled)
        E->>H: Handler 1 (async)
        E->>H: Handler 2 (async)
        E->>H: Handler 3 (async)
        H-->>E: Results (success or error)
    end

    rect rgb(243, 229, 245)
        Note over E: 4. Error Handling
        alt errorIsolation: true
            E->>E: Log errors, continue
        else errorIsolation: false
            E->>App: Throw AggregateError
        end
    end

    E-->>App: Promise resolves
```
