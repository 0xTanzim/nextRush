...
description: Complete guide to all packages in the NextRush ecosystem, their responsibilities, and how they work together.
---
...
```mermaid
flowchart TB
    subgraph Meta["nextrush (Meta Package)"]
        NR[Re-exports all essentials]
    end

    subgraph CoreGroup["Core Packages"]
        direction LR
        CORE["@nextrush/core<br/>Application, Middleware"]
        ROUTER["@nextrush/router<br/>Radix Tree Routing"]
        ERRORS["@nextrush/errors<br/>HTTP Error Classes"]
    end

    subgraph AdapterGroup["Runtime Adapters"]
        direction LR
        NODE["adapter-node"]
        BUN["adapter-bun"]
        DENO["adapter-deno"]
    ...
    subgraph Foundation["Foundation"]
        TYPES["@nextrush/types<br/>Zero-dependency type definitions"]
    end

    NR --> CORE & ROUTER & ERRORS
    CORE & ROUTER & ERRORS --> NODE & BUN & DENO & EDGE
    NODE & BUN & DENO & EDGE --> TYPES
    CORE --> TYPES
    ROUTER --> TYPES
    ERRORS --> TYPES

    style Meta fill:transparent,stroke:#10b981,stroke-width:2px
    style CoreGroup fill:transparent,stroke:#6366f1,stroke-width:2px
    style AdapterGroup fill:transparent,stroke:#f59e0b,stroke-width:2px
    style Foundation fill:transparent,stroke:#8b5cf6,stroke-width:2px,stroke-dasharray:5 5
```
...
**Purpose:** Plugin that connects DI, decorators, and router with auto-discovery.
...
// Auto-discover controllers from ./src
