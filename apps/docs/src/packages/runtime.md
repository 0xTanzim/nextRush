...
description: Cross-runtime detection and abstractions for NextRush - detect Node.js, Bun, Deno, or Edge environments.
---
...
```mermaid
flowchart TB
    subgraph Detection["detectRuntime()"]
        START((Start))
        BUN{Bun global?}
        DENO{Deno global?}
        CF{Cloudflare UA?}
        ...
        BUN -->|Yes| R_BUN["'bun'"]
        BUN -->|No| DENO
        DENO -->|Yes| R_DENO["'deno'"]
        DENO -->|No| CF
        CF -->|Yes| R_CF["'cloudflare-workers'"]
        CF -->|No| VERCEL
        VERCEL -->|Yes| R_VE["'vercel-edge'"]
        VERCEL -->|No| NODE
        NODE -->|Yes| R_NODE["'node'"]
        NODE -->|No| EDGE
        EDGE -->|Yes| R_EDGE["'edge'"]
        EDGE -->|No| UNK
    end

    style Detection fill:transparent,stroke:#6366f1,stroke-width:2px
```
