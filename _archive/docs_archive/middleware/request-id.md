...
Incoming Request
     │
     ├─ Has trusted X-Request-ID header?
     │         │
     │         ├─ Yes → Validate format → Use if valid
     │         │                │
...
┌──────────────────────────────────────────────┐
│         Request-ID Middleware                │
│  ctx.state.requestId = validated/generated   │
│  Response header: X-Request-ID: abc123...    │
└──────────────────────────────────────────────┘
     │
     ▼
  Continue with unique, safe ID
```
...
requestId({
  ...
  // Trust upstream X-Request-ID headers
...
| `trustProxy` | `boolean` | `true` | Trust upstream headers |
...
import {
  ...
  isValidAlphanumeric, // Alphanumeric with hyphens/underscores
  permissiveValidator, // Accept any non-empty string
...
```typescript
// Trust proxy (load balancer, API gateway)
...
type RequestIdValidator = (id: string) => boolean;
```
...
### Mistake 2: Trusting Proxies Without Validation
...
- [ ] **Trust proxy configured**: Only trust if behind load balancer
