...
/**
 * 🚀 NextRush v2 Complete Benchmark Suite
 *
 * Enterprise-grade benchmarking tool for comparing NextRush against
 ...
 *   pnpm bench:all --frameworks nextrush,hono  # Specific frameworks
 *   pnpm bench:all --label "v2.1"     # Add label
 *
...
const AVAILABLE_FRAMEWORKS = ['nextrush', 'express', 'fastify', 'koa', 'hono'];
...
function generateMarkdownReport(results, config, timestamp) {
  ...
  lines.push('# 🚀 NextRush v2 Complete Benchmark Report\n');
  ...
  // Comparison with NextRush
  const nextrush = results.find((r) => r.framework === 'nextrush');
  if (nextrush) {
    lines.push('## 📈 NextRush vs Competition\n');
    lines.push('| Framework | RPS Difference | Performance |');
    lines.push('|-----------|----------------|-------------|');

    for (const r of sorted) {
      if (r.framework === 'nextrush') continue;
      const diff = ((nextrush.summary.avgRps / r.summary.avgRps - 1) * 100).toFixed(1);
  ...
  // Footer
  lines.push('---\n');
  lines.push('*Generated by NextRush Complete Benchmark Suite v2.0*\n');
...
function printResults(results, config) {
  ...
  // Show NextRush comparison
  const nextrush = results.find((r) => r.framework === 'nextrush');
  if (nextrush) {
    console.log(`${c.cyan}📊 NextRush Performance:${c.reset}`);
    for (const r of sorted) {
      if (r.framework === 'nextrush') continue;
      const diff = ((nextrush.summary.avgRps / r.summary.avgRps - 1) * 100);
...
async function main() {
  ...
  console.log(`${c.magenta}║${c.reset}            ${c.bold}🚀 NextRush v2 Complete Benchmark Suite${c.reset}                         ${c.magenta}║${c.reset}`);
  console.log(`${c.magenta}║${c.reset}                                                                              ${c.magenta}║${c.reset}`);
  console.log(`${c.magenta}╚══════════════════════════════════════════════════════════════════════════════╝${c.reset}`);
  console.log('');
  console.log(`${c.dim}Timestamp: ${timestamp}${c.reset}`);
  console.log(`${c.dim}Mode: ${config.quick ? 'Quick' : 'Full'} (${config.duration}s per test)${c.reset}`);
  console.log(`${c.dim}Frameworks: ${config.frameworks.join(', ')}${c.reset}`);
  if (config.label) {
    console.log(`${c.dim}Label: ${config.label}${c.reset}`);
  }
  console.log('');
  ...
  console.log(`   • NextRush only: ${c.cyan}pnpm bench:nextrush${c.reset}`);
  console.log(`   • Compare history: ${c.cyan}pnpm bench:nextrush:compare${c.reset}`);
  console.log('');
}
