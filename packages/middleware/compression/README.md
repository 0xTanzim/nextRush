...
app.use(compression({
  ...
  gzip: true,
  deflate: true,
  brotli: true,  // Only works in Node.js/Bun; disabled automatically elsewhere
...
This adds random-length padding to prevent compression ratio analysis attacks.
...
app.use(async (ctx) => {
  ...
  if (wasCompressed(ctx)) {
    ...
    console.log(`Compressed with ${info?.encoding}: ${info?.ratio * 100}%`);
  }
});
```
...
import {
  ...
  compressData,       // Returns just Uint8Array
  compressToBuffer,   // Returns Node.js Buffer

  // Content negotiation
  parseAcceptEncoding,   // Parse Accept-Encoding header
  negotiateEncoding,     // Select best encoding
  selectEncoding,        // Simple encoding selection
...
Brotli requires Node.js zlib and is automatically disabled in unsupported runtimes.
