/**
 * @nextrush/openapi - Public API surface test
 *
...
import * as openapiApi from '../index';
import type { OpenApiDocument, OpenApiInfo, OpenApiOptions, SchemaConverter } from '../index';

describe('Public API surface (runtime exports)', () => {
  it('exports exactly the intended runtime symbols', () => {
    const actualExports = Object.keys(openapiApi).sort();

    // SEALED: intentional public runtime API surface.
    const expectedRuntime = ['openapi', 'generateDocument', 'toOpenApiPath', 'extractPathParams'].sort();
...
describe('Public API surface (type-only exports)', () => {
  it('the type-only surface stays importable from the barrel', () => {
    ...
    type Surface = [OpenApiDocument, OpenApiInfo, OpenApiOptions, SchemaConverter];
