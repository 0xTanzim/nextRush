...
import { createEvents, EventEmitter, events, MAX_EVENT_NAME_LENGTH, VALID_PROPERTY_NAME } from '../index';
...
describe('events() extension', () => {
  /** Minimal fake ExtensionContext capturing decorate() calls. */
  function fakeCtx() {
    const decorate = vi.fn();
    return { ctx: { decorate } as unknown as Parameters<ReturnType<typeof events>['setup']>[0], decorate };
  }

  it('should create an extension with correct name and lifecycle', () => {
    const ext = events();
    expect(ext.name).toBe('events');
    expect(typeof ext.setup).toBe('function');
    expect(typeof ext.destroy).toBe('function');
  });

  it('should decorate the app with the emitter under the default property name', () => {
    const { ctx, decorate } = fakeCtx();
    events().setup(ctx);
    expect(decorate).toHaveBeenCalledWith('events', expect.any(EventEmitter));
  });

  it('should use a custom property name', () => {
    const { ctx, decorate } = fakeCtx();
    events({ propertyName: 'bus' }).setup(ctx);
    expect(decorate).toHaveBeenCalledWith('bus', expect.any(EventEmitter));
  });

  it('should pass emitter options through', async () => {
    const onError = vi.fn();
    const { ctx, decorate } = fakeCtx();
    events<TestEvents>({ onError }).setup(ctx);
    const emitter = decorate.mock.calls[0]![1] as EventEmitter<TestEvents>;
...
describe('Property Name Validation', () => {
  it('should accept valid property names', () => {
    expect(() => events({ propertyName: 'events' })).not.toThrow();
    expect(() => events({ propertyName: 'myEvents' })).not.toThrow();
    expect(() => events({ propertyName: '_events' })).not.toThrow();
    expect(() => events({ propertyName: '$events' })).not.toThrow();
    expect(() => events({ propertyName: 'events123' })).not.toThrow();
  });

  it('should reject invalid property names', () => {
    expect(() => events({ propertyName: '123events' })).toThrow(TypeError);
    expect(() => events({ propertyName: 'my-events' })).toThrow(TypeError);
    expect(() => events({ propertyName: 'my events' })).toThrow(TypeError);
    expect(() => events({ propertyName: '' })).toThrow(TypeError);
  });

  it('should accept JavaScript reserved words as valid identifiers', () => {
    expect(() => events({ propertyName: 'constructor' })).not.toThrow();
    expect(() => events({ propertyName: '__proto__' })).not.toThrow();
    expect(() => events({ propertyName: 'prototype' })).not.toThrow();
  });
});
...
describe('Security and Edge Cases', () => {
  ...
  it('should handle destroy() being called multiple times', () => {
    const ext = events<TestEvents>();
    const decorate = vi.fn();
    ext.setup({ decorate } as unknown as Parameters<typeof ext.setup>[0]);
    const emitter = decorate.mock.calls[0]![1] as EventEmitter<TestEvents>;
    emitter.on('user:created', vi.fn());
