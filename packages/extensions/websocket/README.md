...
// Start server, then attach the WebSocket server to the underlying HTTP server.
// listen() returns a ServerInstance — pass its `.server` (the raw node:http
// Server), not the ServerInstance wrapper itself.
...
Attach to the raw HTTP server to start handling WebSocket connections. Requires a `node:http`
`Server` instance — not NextRush's `ServerInstance` wrapper.
