...
describe.each(drivers)('conformance: runtime behaviors [$name]', (driver) => {
  ...
  // Built-in-specific: this file knows the 5 built-in adapters by name, so it can
  // assert the exact teardown model (edge/serverless have no server lifetime,
  // F-14). The reusable suite only asserts the flag is declared.
  it('#18/#14 built-in teardown model: edge/serverless have no server lifetime', () => {
