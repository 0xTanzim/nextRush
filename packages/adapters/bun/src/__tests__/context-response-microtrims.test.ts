/**
 * @nextrush/adapter-bun — HP-5-web lazy `ctx.raw` regression contract
 *
...
describe('HP-5-web: Bun ctx.raw lazy memoized wrapper', () => {
...
describe('HP-5-web: Bun ctx.signal still combines request signal + timeout', () => {
  it('aborts ctx.signal when the underlying request signal aborts', () => {
    const controller = new AbortController();
    const ctx = new BunContext(makeRequest({ signal: controller.signal }));
