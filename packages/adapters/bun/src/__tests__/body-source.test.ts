...
describe('BunBodySource', () => {
  describe('constructor', () => {
    ...
    it('should handle request with string body - auto content-type', () => {
      // Note: Web Request API auto-sets content-type for string bodies
      const request = new Request('http://localhost/', {
        method: 'POST',
        body: 'hello',
      });
      ...
      // Request automatically sets text/plain for string bodies
  ...
  describe('text()', () => {
    ...
    it('should throw BodyTooLargeError when content-length exceeds limit', async () => {
      ...
      const bodySource = new BunBodySource(request, { limit: 10 });
  ...
  describe('json()', () => {
    ...
    it('should parse from cached buffer when consumed', async () => {
      ...
      // First read as text to consume
      await bodySource.text();

      // Then read as JSON from cache
      const data = await bodySource.json();
  ...
  describe('stream()', () => {
    ...
    it('should throw BodyConsumedError if already consumed', async () => {
      ...
      await bodySource.text(); // Consume first
...
describe('createBunBodySource', () => {
  it('should create BunBodySource instance', () => {
    const request = createRequestWithBody('test');
    const bodySource = createBunBodySource(request);

    expect(bodySource).toBeInstanceOf(BunBodySource);
  });

  it('should pass options', () => {
    const request = createRequestWithBody('test', 'text/plain', 100);
    const bodySource = createBunBodySource(request, { limit: 10 });
