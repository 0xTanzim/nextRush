/**
 * @nextrush/adapter-bun - Context Tests
 *
 * Tests for BunContext using Web API Request/Response.
...
describe('BunContext', () => {
  ...
  describe('constructor', () => {
    ...
    it('should use provided client IP', () => {
      ctx = new BunContext(request, '192.168.1.100');
      expect(ctx.ip).toBe('192.168.1.100');
    });

    it('should extract IP from x-forwarded-for header when proxy is a hop count', () => {
      request = createMockRequest('http://localhost/', {
        headers: { 'x-forwarded-for': '10.0.0.1, 10.0.0.2' },
      });
      ctx = new BunContext(request, undefined, 1);

      expect(ctx.ip).toBe('10.0.0.2');
    });

    it('should extract IP from x-real-ip header when proxy is a hop count', () => {
      request = createMockRequest('http://localhost/', {
...
describe('throw()', () => {
  ...
  beforeEach(() => {
    ctx = new BunContext(createMockRequest());
  });
...
describe('createBunContext', () => {
  it('should create BunContext instance', () => {
    const request = createMockRequest();
    const ctx = createBunContext(request);

    expect(ctx).toBeInstanceOf(BunContext);
  });

  it('should pass client IP', () => {
    const request = createMockRequest();
    const ctx = createBunContext(request, '10.0.0.1');
