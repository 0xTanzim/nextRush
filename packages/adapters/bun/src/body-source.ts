...
export class BunBodySource implements BodySource {
  ...
  async buffer(): Promise<Uint8Array> {
    ...
    // Use Bun's native arrayBuffer() - highly optimized
  ...
  async text(): Promise<string> {
    // If not consumed, use Bun's native text() for better performance
    if (!this._consumed) {
      // Check content-length limit before reading
      if (this.contentLength !== undefined && this.contentLength > this.options.limit) {
        throw new BodyTooLargeError(this.options.limit, this.contentLength);
      ...
      // Check actual size after reading (byte length may differ from string length)
  ...
  async json<T = unknown>(): Promise<T> {
    // If not consumed, use Bun's native json() for better performance
    if (!this._consumed) {
      // Check content-length limit before reading
      if (this.contentLength !== undefined && this.contentLength > this.options.limit) {
        throw new BodyTooLargeError(this.options.limit, this.contentLength);
