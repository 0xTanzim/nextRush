...
describe('EdgeBodySource', () => {
  ...
  describe('text()', () => {
    ...
    it('should throw BodyTooLargeError when content-length exceeds limit', async () => {
      ...
      const bodySource = new EdgeBodySource(request, { limit: 10 });
...
describe('createEdgeBodySource', () => {
  it('should create EdgeBodySource instance', () => {
    const request = createRequestWithBody('test');
    const bodySource = createEdgeBodySource(request);

    expect(bodySource).toBeInstanceOf(EdgeBodySource);
  });

  it('should pass options', () => {
    const request = createRequestWithBody('test', 'text/plain', 100);
    const bodySource = createEdgeBodySource(request, { limit: 10 });
...
describe('F-04a — shared WebBodySource reuse', () => {
  it('EdgeBodySource is the shared runtime WebBodySource', () => {
    expect(EdgeBodySource).toBe(WebBodySource);
  });

  it('createEdgeBodySource is the shared runtime factory', () => {
    expect(createEdgeBodySource).toBe(createWebBodySource);
  });
