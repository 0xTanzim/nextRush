 ...
 *
 * When `trustProxy` is `false` (default), `ctx.ip` is `''` on Edge (no
...
describe('EdgeContext ctx.ip dev-mode warning (P2-3)', () => {
  ...
  it('warns once, in development, on the first read of ctx.ip when trustProxy is false', () => {
    ...
    expect(warnSpy.mock.calls[0]?.[0]).toContain('trustProxy');
  ...
  it('does not warn when trustProxy is true, even in development', () => {
