/**
 * @nextrush/adapter-deno — HP-5-web lazy `ctx.raw` regression contract
 *
...
describe('HP-5-web: Deno ctx.raw lazy memoized wrapper', () => {
...
describe('HP-5-web: Deno ctx.signal still combines request signal + timeout', () => {
  it('aborts ctx.signal when the underlying request signal aborts', () => {
    const controller = new AbortController();
    const ctx = new DenoContext(makeRequest({ signal: controller.signal }));
