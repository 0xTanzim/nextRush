...
- **9.4 (RFC/ADR approval):** **Genuinely open, not closed by this pass.** All four RFCs (RFC-029, RFC-030, RFC-031, RFC-032) and their four ADRs (ADR-0017 through ADR-0020) exist with real content, but every one of them still carries `Status: Draft` (RFCs) / `Status: Proposed` (ADRs) — confirmed by reading each file's own status field directly, not inferred. Per this repo's own governance (`AGENTS.md` §20: "durable architectural decisions land in `docs/RFC/` before a change is archived"; `tdd-workflow.md`'s project addendum: "RFC before implementation... requires an approved RFC"), flipping these to `Approved`/`Accepted` is a maintainer sign-off decision, not something the same automated process that implemented the change should self-certify. **This blocks archiving `harden-security-boundaries` under the repo's own stated rule** until a human with that authority reviews and approves/accepts all four. Recorded here plainly rather than silently marking 9.4 done.
...
**Net closure status:** 9.1, 9.3 (sampled), 9.5, 9.6, 9.7, 9.8 done. 9.2 explicitly not run (disclosed, not fabricated). **9.4 is a genuine, unresolved blocker on archiving this change** until RFC-029 through RFC-032 and ADR-0017 through ADR-0020 receive real maintainer approval — this is not a formality to wave through.
